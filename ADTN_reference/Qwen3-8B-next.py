...
# 统一成一个变量
MODEL_DIR = Path("llama3_tnn_JOINT_L35_34_33_etc_1/checkpoint-136000").resolve()
...
def prepare_qwen_base_datasets(
    ...
    fw_iter = load_dataset("HuggingFaceFW/fineweb-edu", name="sample-10BT", split="train", streaming=True)
    fw_data = list(itertools.islice(fw_iter, 40000))
    ...
    ds_magpie = load_dataset("Magpie-Align/Magpie-Qwen2.5-Pro-300K-Filtered", split="train")
    ds_magpie = ds_magpie.select(range(20000))
    ...
    # Code (Glaive)
    ds_code = load_dataset("glaiveai/glaive-code-assistant-v3", split="train")
    ds_code = ds_code.select(range(5000))
    ds_code = ds_code.map(_norm_glaive_code, num_proc=num_proc, remove_columns=ds_code.column_names)
    # Math (OpenMath)
    ds_math = load_dataset("nvidia/OpenMathInstruct-2", split="train_1M")
    ds_math = ds_math.select(range(5000))
...
def train_specific_layers_jointly(model, tok, base_cfg, *, device, run_dtype, head_dim,
    ...
    for L in target_layers:
        ...
        for name, submodule in layer_module.named_modules():
            ...
            if hasattr(submodule, "bricks"):
                ...
                shape_str = str(tuple(submodule.bricks.shape))
                info_str = f"✅ ADTN | bricks={shape_str}"

                # 收集参数用于后续微调
                for p in submodule.parameters():
                    existing_params_in_layer.append(p)

            # B. 检查是否是 Linear (看有没有 weight)
            elif hasattr(submodule, "weight"):
                shape_str = str(tuple(submodule.weight.shape))
                info_str = f"🔄 Linear | weight={shape_str}"

            # 只有是 Linear 或 ADTN 组件时才打印，避免打印激活函数等无参层
            if info_str:
                # 打印全名，例如 self_attn.q_proj
                print(f"    - {name:<30} : {submodule.__class__.__name__:<25} -> {info_str}")

        # 3. 分支决策
        if found_adtn_in_this_layer:
            print(f"  🛑 [决策] 第 {L} 层已包含 ADTN 结构。")
            print(f"  -> 跳过替换与拟合，直接收集 {len(existing_params_in_layer)} 个参数复用。")
            ...
            print(f"  🚀 [决策] 第 {L} 层为纯 Linear 结构。")
            print("  -> 执行 replace_linear_with_tnn_from_config (MSE Warmup) ...")
    ...
    output_dir_name = f"qwen3_tnn_JOINT_L{layer_str}_2"
...
def main():
    ...
    # ================= Debug 区域 =================
    print("\n--- DEBUG: Offset Map Inspection ---")
    if hasattr(base_cfg_obj, "_adtn_gate_offset_map"):
        raw_map = base_cfg_obj._adtn_gate_offset_map
        print(f"Map found! Total entries: {len(raw_map)}")

        # 打印前几个看看 Key 是字符串还是元组
        keys = list(raw_map.keys())
        print(f"Sample Keys (Raw): {keys[:3]}")
        print(f"Type of Key[0]: {type(keys[0])}")

        # 尝试获取报错的那一层 (Layer 16 或者你报错的 Layer)
        # 假设报错提到的 layer 是 16
        val_str = raw_map.get("(16, 'q')")
        val_tuple = raw_map.get((30, 'q'))
        print(f"Lookup string key \"(16, 'q')\": {val_str}")
        print(f"Lookup tuple key (16, 'q'): {val_tuple}")

        # === 核心修复逻辑：强制转为元组 ===
        print(">>> Fixing Map Keys (String -> Tuple)...")
        new_map = {}
        for k, v in raw_map.items():
            if isinstance(k, str) and k.startswith("("):
                try:
                    # 安全地将字符串 "(16, 'q')" 转为元组 (16, 'q')
                    k_tuple = ast.literal_eval(k)
                    new_map[k_tuple] = int(v)
                except:
                    new_map[k] = v
            else:
                new_map[k] = v
        ...
        # 覆盖 Config 中的 Map
        base_cfg_obj._adtn_gate_offset_map = new_map
        print(f"Map fixed. Now looking up tuple (16, 'q'): {new_map.get((30, 'q'))}")

    else:
        print("❌ WARNING: '_adtn_gate_offset_map' NOT found in config!")

    # 2. 【关键】强制设置全局默认 Offset = 3
    # 这样即使 map 里找不到 layer 16，它也会回退到这个全局值，从而创建正确的 4 Gate 结构
    print("  - Forcing global adtn_gate_offset = 3 (Fallback)")
    base_cfg_obj.adtn_gate_offset = 3
