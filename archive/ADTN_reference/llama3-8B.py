...
def prepare_llama3_datasets(
    ...
    # Pad Token 设置 (Llama 3 使用 <|reserved_special_token_0|> 作为 Pad)
    ...
    # -------------------------------------------------------------------------
    # 加载数据集 (Llama 3 Instruct 全能恢复配方)
    # 总量约 100k
    # -------------------------------------------------------------------------

    print("Loading Alpaca...")
    ds_alpaca = load_from_disk(f"{LOCAL_DATA_DIR}/alpaca_cleaned").map(
        lambda ex: _norm_from_alpaca(ex), num_proc=num_proc, remove_columns=["instruction", "input", "output"]
    )

    # 1. Magpie-Llama-3-Pro (同源对齐: 30k -> 30%)
    # 保持 Llama 3 原汁原味的回答风格，这是最重要的
    ...
    # 2. OpenHermes-2.5 (通用增强: 20k -> 20%)
    # 高质量指令集，替代 Alpaca，提升逻辑、代码和通用任务能力
    print("Loading OpenHermes-2.5 (20k)...")
    ds_hermes = load_from_disk(f"{LOCAL_DATA_DIR}/openhermes_2_5")
    ds_hermes = ds_hermes.map(
        _norm_openhermes,  # 使用 OpenHermes 专用处理函数
        num_proc=num_proc,
        remove_columns=ds_hermes.column_names
    )

    # 3. UltraChat_200k (对话专家: 20k -> 20%)
    # 恢复多轮对话能力
    print("Loading UltraChat 200k (20k)...")
    ds_chat = load_from_disk(f"{LOCAL_DATA_DIR}/ultrachat_200k")
    ds_chat = ds_chat.map(
        _norm_ultrachat,  # 复用 UltraChat 处理函数
        num_proc=num_proc,
        remove_columns=ds_chat.column_names
    )

    # 4. OpenMathInstruct-2 (数学逻辑: 15k -> 15%)
    # 保护逻辑推理能力，防止变傻
    print("Loading OpenMath (15k)...")
    ds_math = load_from_disk(f"{LOCAL_DATA_DIR}/openmath_instruct_2_subset")
    ds_math = ds_math.select(range(100000))
    ds_math = ds_math.map(
        _norm_openmath, num_proc=num_proc,
        remove_columns=ds_math.column_names
    )

    # 5. FineWeb-Edu (知识锚点: 15k -> 15%)
    # 纯文本，防止 PPL 升高
    print("Loading FineWeb-Edu (15k)...")
...
class QuickEvalCallback(TrainerCallback):
    def __init__(self, tokenizer, eval_steps=50, num_mmlu_samples=5, num_gsm8k_samples=3):
        self.tokenizer = tokenizer
        self.eval_steps = eval_steps

        # --- 准备数据 (从本地加载) ---
        print("Loading Mini-Eval Datasets from Local Disk...")
        ...
        for sub in self.mmlu_subsets:
            ...
            if os.path.exists(path):
                # 加载并只取前 N 条
                self.mmlu_data[sub] = load_from_disk(path).select(range(num_mmlu_samples))
            else:
                print(f"Warning: Local MMLU data for {sub} not found at {path}. Skipping.")

        # 2. GSM8K
        gsm8k_path = os.path.join(LOCAL_DATA_DIR, "eval_gsm8k_test")
        if os.path.exists(gsm8k_path):
            self.gsm8k_data = load_from_disk(gsm8k_path).select(range(num_gsm8k_samples))
        else:
            print(f"Warning: Local GSM8K data not found at {gsm8k_path}. Skipping.")
            self.gsm8k_data = []

        # 预计算选项 Token ID (保持不变)
        # 不同的 tokenizer 对空格处理不同，这里做一个简单的容错
        candidates = [" A", " B", " C", " D", "A", "B", "C", "D"]
        self.choice_tokens = []
        for char in ["A", "B", "C", "D"]:
            # 尝试带空格和不带空格，取编码后长度为1的那个，或者取最后一个token
            # Llama tokenizer 通常 " A" 是单个token，"A" 也是。为了保险，我们用 " A"
            t_id = tokenizer.encode(" " + char, add_special_tokens=False)[-1]
            self.choice_tokens.append(t_id)

    def _format_mmlu_prompt(self, example):
        """MMLU 0-shot 格式"""
        prompt = f"Question: {example['question']}\nChoices:\nA. {example['choices'][0]}\nB. {example['choices'][1]}\nC. {example['choices'][2]}\nD. {example['choices'][3]}\nAnswer:"
        return prompt

    def _evaluate_mmlu(self, model, device):
        """计算 MMLU 选择题准确率 (基于 Logits)"""
        results = {}
        model.eval()
        ...
        with torch.no_grad():
            for sub, ds in self.mmlu_data.items():
                ...
                total = 0
                for ex in ds:
                    prompt = self._format_mmlu_prompt(ex)
                    ...
                    # 提取 A, B, C, D 的分数
                    choice_logits = last_token_logits[self.choice_tokens]
                    pred_idx = torch.argmax(choice_logits).item()  # 0, 1, 2, 3

                    if pred_idx == ex['answer']:
                        correct += 1
                    total += 1

                results[f"MMLU/{sub}"] = correct / total

        model.train()
        return results

    def _generate_gsm8k(self, model, device):
        """生成 GSM8K 样例 (视觉检查)"""
        print("\n\n=== GSM8K Generation Check ===")
        model.eval()

        # 临时调整 tokenizer padding side 为左，方便 generate
        old_pad_side = self.tokenizer.padding_side
        self.tokenizer.padding_side = "left"

        with torch.no_grad():
            for i, ex in enumerate(self.gsm8k_data):
                ...
                gen_out = model.generate(
                    **inputs,
                    max_new_tokens=256,
                    do_sample=False,  # 使用 Greedy Search 看最稳定的能力
                    pad_token_id=self.tokenizer.pad_token_id
                )

                output_text = self.tokenizer.decode(gen_out[0], skip_special_tokens=True)
                # 截取 Question 之后的部分
                answer_part = output_text[len(question):].strip()

                print(f"--- GSM8K Ex {i + 1} ---")
                print(f"Q: {ex['question'][:100]}...")  # 只打印前100字符
                print(f"Gold: {ex['answer'].split('####')[-1].strip()}")  # 打印标准答案数值
                print(f"Pred: {answer_part.replace(chr(10), ' ')}")  # 打印预测，去除换行方便看
                print("---------------------")
    ...
    def on_step_end(self, args, state, control, model, **kwargs):
        ...
        if state.global_step > 0 and state.global_step % self.eval_steps == 0:
            print(f"\n[Step {state.global_step}] Running Quick Eval...")

            # 1. 测 MMLU
            mmlu_res = self._evaluate_mmlu(model, args.device)
            print(f"MMLU Acc: {mmlu_res}")

            # 2. 测 GSM8K 生成
            self._generate_gsm8k(model, args.device)
...
def train_specific_layers_jointly(model, tok, base_cfg, *, device, run_dtype, head_dim,
    ...
    quick_eval_callback = QuickEvalCallback(
        ...
        num_mmlu_samples=10,  # 每个学科测 10 题
        num_gsm8k_samples=2  # 每次生成 2 个数学题看看
    )
...
def main():
    ...
    # 5) 定义你想要压缩并联合训练的层
    # 例如：只压缩最后 3 层进行联合训练
    layers_to_compress = [31, 30, 29, 28, 27, 26, 25, 24, 23, 22, 21, 20, 19, 18, 17]
