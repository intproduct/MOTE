...
def _norm_magpie_qwen(ex: Dict) -> Dict:
    """
    处理 Magpie-Qwen2-Pro。
    数据结构: 'conversations' list
    """
    raw_convs = ex.get("conversations", [])
    if not raw_convs: return {"messages": []}

    # 检查 System Prompt
    if raw_convs[0]["role"] != "system":
        # 插入默认 System
        raw_convs.insert(0, {"role": "system", "content": SYSTEM_DEFAULT})
...
class DataCollatorForCausalLMWithLabels:
    ...
    def __call__(self, features: List[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
        ...
        batch = {}
        # 注意：Llama 3 的 pad_token_id 必须正确设置
...
def prepare_qwen_datasets(
        repo_id: str,  # e.g., "Qwen/Qwen2.5-7B-Instruct"
        block_size: int = 4096,  # Qwen 支持 32k+，但训练 4096 够用了
        num_proc: int = 8,
    ...
    # --- Qwen Tokenizer 特殊处理 ---
    # Qwen 2.5 的 pad_token 通常是 <|endoftext|> (151643) 或者 <|im_end|>
    # 如果没设置，我们需要手动设，防止 Collator 报错
    ...
    if tokenizer.chat_template is None:
        print("Warning: No chat template found. Setting default ChatML.")
    ...
    # A. Magpie-Qwen (同源对齐)
    print("Loading Magpie-Qwen2-Pro...")
    ds_magpie = load_dataset("Magpie-Align/Magpie-Qwen2-Pro-200K-Filtered", split="train")
    ds_magpie = ds_magpie.select(range(40000))  # 取 40k
    ds_magpie = ds_magpie.map(
        _norm_magpie_qwen, num_proc=num_proc,
        remove_columns=ds_magpie.column_names
    )

    # B. Evol-CodeAlpaca (强化代码)
    print("Loading Evol-CodeAlpaca...")
    ds_code = load_dataset("theblackcat102/evol-codealpaca-v1", split="train")
    ds_code = ds_code.select(range(25000))  # 取 25k
    ds_code = ds_code.map(
        _norm_evol_code, num_proc=num_proc,
        remove_columns=ds_code.column_names
    )

    # C. OpenMathInstruct-2 (强化数学)
    print("Loading OpenMathInstruct-2...")
    ds_math = load_dataset("nvidia/OpenMathInstruct-2", split="train_1M")
    ds_math = ds_math.select(range(20000))  # 取 20k
    ds_math = ds_math.map(
    ...
    fw_data = list(itertools.islice(fw_iter, 15000))  # Qwen 知识量大，取 15k
    ...
    # -------------------------------------------------------------------------
    # 3. Tokenization & Return
    # -------------------------------------------------------------------------
    print("Tokenizing with Qwen ChatML template...")
    # 复用之前的 build_encoder
...
def progressive_train_from_last_layer(model, tok, base_cfg, *, device, run_dtype, head_dim,
    ...
    for L in range(start, end_layer - 1, -1):
        ...
        args = TrainingArguments(
            output_dir=f"llama_tnn_L{L:02d}_ckpts_layer_3",
            per_device_train_batch_size=4,
            gradient_accumulation_steps=8,
            learning_rate=5e-5,
        ...
        if replaced_ok % 4 == 0:
            ...
            args_all = TrainingArguments(
                output_dir=f"llama_tnn_ALL_ADTN_after{replaced_ok:02d}_3",
                per_device_train_batch_size=4,
                gradient_accumulation_steps=8,
                learning_rate=5e-5,
...
def main():
    # 1) 数据准备（保留你的实现）
    tok, train_ds, eval_ds, collator = prepare_qwen_datasets(
        repo_id=REPO)
